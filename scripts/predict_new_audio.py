...
def main() -> None:
    ...
    input_group.add_argument(
        "--features", type=Path, help="Precomputed WST feature array with shape (n, 221, 375)"
    )
    input_group.add_argument(
        "--audio-dir",
        type=Path,
        help="Folder of `.wav` files to convert to WST features before prediction",
    )
    parser.add_argument(
        "--model",
        type=Path,
        default=Path("models/pretrained_cae_wst_latent24_structural_audio.h5"),
        help="Trained `.keras` or `.h5` CAE",
    )
    parser.add_argument(
        "--classifier",
        type=Path,
        required=True,
        help="Saved joblib classifier from evaluate_feature_map_classifier.py",
    )
    parser.add_argument(
        "--reference-maps",
        type=Path,
        required=True,
        help="reference_maps.npz from evaluate_feature_map_classifier.py",
    )
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as handle:
        ...
        for record, score_row, prediction, probability in zip(
            records, scores, predictions, probabilities, strict=True
        ):
            writer.writerow(
                [
                    record.source_file,
